...
// This is the "byte_strobe" test except the PPU command processor is put into
// an infinite loop, generating instruction fetch traffic to ERAM at the same
// time the CPU is executing from it and writing to it.
...
ppu_instr_t program[64];

int main() {
	volatile uint32_t *buf32 = (volatile uint32_t*)buf;
	volatile uint16_t *buf16 = (volatile uint16_t*)buf;

	vuart_puts("Starting CPROC\n");
	ppu_instr_t *p = &program[0];
	p += cproc_jump(p, &program[0]);
	cproc_put_pc(&program[0]);
	ppu_start(false);
